...
for alpha in "${alphas[@]}"; do
    batch_name="Targeted_NonAnta_MA_DAB_${alpha}"
...
# Concatenate all Targeted_NonAnta_MA_DAB_*_BitError_Combinations.csv files into one, keeping only the first header
output_file="Targeted_NonAnta_MA_DAB_BitError_Combinations.csv"
first=1
> "$output_file"
for alpha in "${alphas[@]}"; do
    file="Targeted_NonAnta_MA_DAB_${alpha}_BitError_Combinations.csv"
    if [ -f "$file" ]; then
        if [ $first -eq 1 ]; then
...
python3 ../batch_matrix_generator.py Targeted_NonAnta_MA_DAB
