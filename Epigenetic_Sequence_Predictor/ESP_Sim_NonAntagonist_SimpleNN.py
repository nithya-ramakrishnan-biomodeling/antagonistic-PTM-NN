# Compared to ESP_Sim_Antagonist_SimpleNN.py, this file uses 'gamma' instead of 'mu' for the non-antagonistic probability.
# The changes are specifically in:
#   - The mother sequence generation function: uses 'gamma' (generate_non_anta_mom instead of the generate_mom).
#   - The Dataframe class: uses 'gamma' as a parameter and passes it to mother generation.
#   - The run_sim function: uses 'gamma' for both training and test Dataframe creation.
...
def generate_non_anta_mom(alpha, beta, gamma, n=100, start_as=2):
    """
    This function generates one sequence of the mother based on alpha, beta, gamma and n values.
    the alpha and beta values are used to create the A part of the mother and the gamma is used to create the B.
    here, gamma refers to the non-antagonistic probability (can also be thought of as the correlation between A and B)
    ...
    :param gamma: The probability that the element in the B sequence is 1 (only if A is 1).
    ...
    for i in range(mother_sequence_A.size):
        if mother_sequence_A[i] == 0 and mother_sequence_B[i] == 0:
            ...
            mother_sequence[i] = 3  # mother_sequence_A[i] == 1 and mother_sequence_B[i] == 1

    return mother_sequence, mother_sequence_A, mother_sequence_B
...
class Dataframe:
    ...
    def __init__(self, alpha, beta, gamma, rho, n_moms, mom_length, mom_start=2):
        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma
        self.rho = rho
        ...
        for i in range(n_moms):
            ...
            new_mom, _, _ = generate_non_anta_mom(alpha=self.alpha,
                                                  beta=self.beta,
                                                  gamma=self.gamma,
                                                  n=mom_length,
                                                  start_as=mom_start)
...
def run_sim(sim_id, seed=42,
            alpha=0.8, beta=0.8, gamma=0.5, rho=0.5, n_samples=100000, n_samples_test=30000, seq_length=100,
            input_size=200, hidden_size=int(1.5 * 100), output_size=100, num_layers=5,
    ...
    :param alpha: The probability that the next element node in the sequence is 1 given that the current node is 1.
    :param beta: The probability that the next element node in the sequence is 0 given that the current node is 0.
    :param gamma: The probability that the B part of the mom is 1 if the A part of the mom is 1.
    ...
    data = Dataframe(alpha, beta, gamma, rho, n_samples, seq_length)
    ...
    # Test the model.
    test_data = Dataframe(alpha, beta, gamma, rho, n_samples_test, seq_length)
