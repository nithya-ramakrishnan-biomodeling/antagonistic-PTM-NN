import time
from Epigenetic_Sequence_Predictor.ESP_Sim_Antagonist_Range_SimpleNN import run_sim

batch_name = 'Range_Test_Simple'

sim_start = time.time()
sim_name, initial_biterror = run_sim(sim_id=batch_name, seed=42,
                                     alpha_min=0.0, alpha_max=1.0,
                                     beta_min=0.0, beta_max=1.0,
                                     mu_min=0.0, mu_max=1.0,
                                     rho=0.5,
                                     n_samples=100000, n_samples_test=30000,
                                     seq_length=100,
                                     input_size=200, hidden_size=int(1.5 * 100), output_size=100,
